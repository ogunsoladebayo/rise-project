...
export const getTopUsers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const users = await db.em.getConnection().execute(`WITH LatestComments AS (SELECT c.author_id,
                               MAX(c.created_at) AS latestCommentCreatedAt,
                               MAX(c.content)    AS latestCommentContent
                        FROM comment c
...
SELECT u.id,
       ...
       lc.latestCommentContent AS latestComment
FROM "user" u
         LEFT JOIN
     post p ON u.id = p.author_id
         LEFT JOIN
     LatestComments lc ON u.id = lc.author_id
